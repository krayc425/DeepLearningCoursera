...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(a_C, [n_C, n_H * n_W, m])
    a_G_unrolled = tf.reshape(a_G, [n_C, n_H * n_W, m])
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = 1 / (4 * (n_C * n_H * n_W) ** 2) * tf.reduce_sum(tf.squared_difference(GS, GG))
