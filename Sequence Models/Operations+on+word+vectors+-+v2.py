...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    ba_norm_sq = np.sum(bias_axis * bias_axis)
    
    mu_B = (np.dot(mu, bias_axis) / ba_norm_sq) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (np.dot(e_w1, bias_axis) / ba_norm_sq) * bias_axis
    e_w2B = (np.dot(e_w2, bias_axis) / ba_norm_sq) * bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    term_1 = np.sqrt(np.abs(1 - np.sum(mu_orth * mu_orth)))
    
    corrected_e_w1B = term_1 * ((e_w1B - mu_B) / np.abs((e_w1 - mu_orth) - mu_B))
    corrected_e_w2B = term_1 * ((e_w2B - mu_B) / np.abs((e_w2 - mu_orth) - mu_B))
